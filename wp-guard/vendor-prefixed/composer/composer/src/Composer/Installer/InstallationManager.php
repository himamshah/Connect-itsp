...
use Anystack\WPGuard\V001\Composer\DependencyResolver\Operation\MarkAliasInstalledOperation;
use Anystack\WPGuard\V001\Composer\DependencyResolver\Operation\MarkAliasUninstalledOperation;
use Anystack\WPGuard\V001\Composer\Downloader\FileDownloader;
...
class InstallationManager
{
    ...
    private function executeBatch(InstalledRepositoryInterface $repo, array $operations, array $cleanupPromises, bool $devMode, bool $runScripts, array $allOperations): void
    {
        ...
        foreach ($operations as $index => $operation) {
            ...
            if (!$promise instanceof PromiseInterface) {
                $promise = \Anystack\WPGuard\V001\React\Promise\resolve(null);
            }
    ...
    private function runCleanup(array $cleanupPromises): void
    {
        ...
        foreach ($cleanupPromises as $cleanup) {
            $promises[] = new \Anystack\WPGuard\V001\React\Promise\Promise(static function ($resolve, $reject) use ($cleanup): void {
                $promise = $cleanup();
                if (!$promise instanceof PromiseInterface) {
                    $resolve();
                } else {
